...
def validar_rut(rut_string):
    """
    Valida formato con REGEX y luego calcula el Dígito Verificador.
    Acepta: 12.345.678-K, 12345678-k, 12345678k
    """
    if not rut_string:
        return

    # 1. Regex: Valida estructura visual
    ...
    if dvr != dv:
        raise ValidationError(f"RUT inválido. El dígito verificador no corresponde.")
